...
namespace BooleanWidth.Algorithms.BooleanWidth.Linear.Heuristics
{
    ...
    public static class SadiaHeuristics
    {
        ...
        private static int LeastUncommonNeighbor(Datastructures.Graph graph, BitSet left, BitSet right)
        {
        ...
        private static int RelativeNeighborhood(Datastructures.Graph graph, BitSet left, BitSet right)
        {
            ...
            foreach (int v in nl)
            {
                ...
                double externalV = (nv - nl).Count;

                // Number of vertices inside of both N(left) * right and N(v) * right
                double internalV = nv.Count - externalV;

                if (internalV == 0) internalV = 1; // cant devide by zero!

                // Possibly update the minimum ratio found so far
                double ratio = externalV / internalV;
        ...
        private static int LeastCutValue(Datastructures.Graph graph, BitSet left, BitSet right)
        {
